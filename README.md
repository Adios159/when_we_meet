...
## 왜 완전한 바닐라 JS가 아니라 FastAPI 백엔드가 필요한가

"여러 사람이 같은 페이지를 공유해서 본다"는 건 데이터가 한 곳(서버)에
모여야 가능한 일입니다. 브라우저의 `localStorage` 등은 그 브라우저
안에서만 유효해서, 다른 사람 화면으로 절대 전달되지 않습니다.
그래서 프론트엔드는 바닐라 JS로 만들고, "제출 데이터를 모아서 겹치는
시간만 계산해 돌려주는" 부분만 최소한의 FastAPI 서버로 구현했습니다.
